...
class Executor:
    ...
    async def _get(self, path, params=None):
        ...
        r = await self._http.get(f"{self.base}{path}?{qs}")
        if not r.is_success:
            body = r.text[:200]
            log.error(f"GET {path} → {r.status_code}: {body}")
            raise Exception(f"GET {path} → {r.status_code}: {body}")
        return r.json()

    async def _post(self, path, params=None):
        qs = self._sign(params or {})
        r = await self._http.post(f"{self.base}{path}?{qs}")
        if not r.is_success:
            body = r.text[:200]
            log.error(f"POST {path} → {r.status_code}: {body}")
            raise Exception(f"POST {path} → {r.status_code}: {body}")
        return r.json()

    async def _delete(self, path, params=None):
        qs = self._sign(params or {})
        r = await self._http.delete(f"{self.base}{path}?{qs}")
        if not r.is_success:
            body = r.text[:200]
            log.warning(f"DELETE {path} → {r.status_code}: {body}")
    ...
    async def load_filters(self, sym: str):
        try:
            r = await self._http.get(f"{self.base}/fapi/v1/exchangeInfo")
            r.raise_for_status()
            ...
            log.warning(f"Filtre: {e}")

    def _rqty(self, q):
        s = self._qty_step or 10 ** -self._qty_prec
        v = math.floor(q / s) * s
        return f"{round(v, self._qty_prec):.{self._qty_prec}f}"

    def _rprice(self, p):
        s = self._price_step or 10 ** -self._price_prec
        v = round(round(p / s) * s, self._price_prec)
        return f"{v:.{self._price_prec}f}"
    ...
    async def set_margin_type(self, sym, mt="ISOLATED"):
        try:
            await self._post("/fapi/v1/marginType", {"symbol": sym, "marginType": mt})
        except:
            pass  # already set
    ...
    async def klines(self, sym, limit=80):
        for base in [self.base, LIVE]:
            try:
                ...
                rows = r.json()
                if rows:
                    log.info(f"{len(rows)} kline yüklendi ({base})")
                    return [{"o": float(k[1]), "h": float(k[2]), "l": float(k[3]), "c": float(k[4]), "v": float(k[5]), "t": int(k[0])} for k in rows]
            except Exception as e:
    ...
    async def stop_order(self, sym, side, qty, stop_price):
        ...
        if self.demo:
            ...
            try:
                ...
                log.warning(f"STOP emri başarısız (RT ile korunur): {e}")
                return {"orderId": 0}
        else:
            return await self._post("/fapi/v1/order", {
    ...
    async def cancel_all_orders(self, sym):
        try:
            return await self._delete("/fapi/v1/allOpenOrders", {"symbol": sym})
        except:
            pass
